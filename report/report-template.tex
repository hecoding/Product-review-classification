...
\begin{table}[h]
	...
	\end{tabular}
	\caption{Final results: test set performance and training time}
\end{table}

The results of our mini project are summarized in Table~\ref{tab:results}. The most noticeable matter in this table is, that we reduced the training time significantly by changing the recurrent unit from LSTMs to GRUs. Simultaneously the performance was also improved.

\subsection{LSTM vs GRU}
Our main difference from the base line, after all the experimentation, is the switch from Long Short-Term Memory units to Gated Recurrent Units. The gain was evident performance and time-wise.
