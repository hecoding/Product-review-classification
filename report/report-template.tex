...
\usepackage{a4wide} 				%% kleinere SeitenrÊnder
...
\title{Binary Classification of Amazon Product Reviews based on Recurrent Neural Networks\\
...
Filtering out irrelevant Amazon product reviews and rather focussing on relevant product reviews is an improvement where both Amazon and consumers mutually benefit from. 
To tackle this task we implemented a model relying on recurrent neural networks (RNN) which is performing a binary classification of a products review either as good or as bad.
Our results are compared to similar approaches. Additionally we demonstrate that using GRU instead of LSTMs improves the prediction accuracy to 75.1\% respectively 77.5\% for bad respectively good reviews, whereas a model used LSTMs reaches to 74.7\% respectively 76.3\%.

Furthermore we evaluate the implications and advantages as well as disadvantages of variations of e.g. the dimensions of the architecture, the consideration of certain features and number of RNN layers.  
