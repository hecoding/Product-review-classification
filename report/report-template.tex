...
\texttt{some ideas}
  ...
  In our mini project we tackle the problem of assessing review helpfulness using Deep Learning. We made use of Amazon reviews and Recurrent Neural Networks (RNNs) for the task, which gave us decent performance.
  
 In our mini project we wanted to take useful previous work into account, so we also took relevant papers that used the same dataset and tried to replicate their work to learn what are the drawbacks and problems one can run into when trying to prove the results true.
 
 After successfully validating the statements of our references, we discuss some significant variations and improvements.
\end{abstract}
